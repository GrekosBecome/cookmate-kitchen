...
export const Step1Diet = ({
  ...
  return <div className="space-y-8 max-w-2xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-4 pt-6">
        {mealOptions.map(option => <MealPreferenceCard key={option.id} icon={option.icon} label={option.label} selected={selectedPreferences.includes(option.id)} onToggle={() => onTogglePreference(option.id)} />)}
      </div>
